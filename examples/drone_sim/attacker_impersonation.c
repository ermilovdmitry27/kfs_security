...
#define IMPERSONATION_INTERVAL_MS 700

PROCESS(attacker_impersonation_proc, "Attacker impersonation");
AUTOSTART_PROCESSES(&attacker_impersonation_proc);
...
static void send_impersonation_to_target(attack_target_t *target, void *ctx);
...
PROCESS_THREAD(attacker_impersonation_proc, ev, data)
{
  ...
  mesh_open(&mesh, current_channel, &mesh_cb);
  attack_targets_init();
  etimer_set(&timer, (CLOCK_SECOND * IMPERSONATION_INTERVAL_MS) / 1000);

  while(1) {
    PROCESS_WAIT_EVENT_UNTIL(etimer_expired(&timer));
    attack_targets_for_each(send_impersonation_to_target, NULL);
...
static void
send_impersonation_to_target(attack_target_t *target, void *ctx)
{
  sensor_msg_t msg;
  (void)ctx;
